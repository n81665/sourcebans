...
if (!defined("IN_SB")) {
    die("Тебя не должно быть здесь. Следуйте только по ссылкам.!");
}
/**
...
{
    global $userbank;
    AddTab("Главная", "index.php?p=home", "Эта страница показывает обзор ваших банов и серверов.");
    AddTab("Серверы", "index.php?p=servers", "Все ваши серверы и их статус можно посмотреть здесь.");
    AddTab("Баны", "index.php?p=banlist", "Все баны в базе данных можно посмотреть здесь.");
    if ($GLOBALS['config']['config.enablecomms'] == "1") {
        AddTab("Муты", "index.php?p=commslist", "Все баны на связь (такие как чат и голосовую блокировку) в базе данных можно посмотреть здесь.");
    }
    if ($GLOBALS['config']['config.enablesubmit']=="1") {
        AddTab("Жалоба на игрока", "index.php?p=submit", "Здесь вы можете представить демо или скриншот подозреваемого читера. Затем он будет рассмотрен одним из администраторов.");
    }
    if ($GLOBALS['config']['config.enableprotest']=="1") {
        AddTab("Протест бана", "index.php?p=protest", "Здесь вы можете обжаловать свой запрет. И докажите свое мнение о том, почему вы должны быть разбаннены.");
    }
    if ($userbank->is_admin()) {
        AddTab("Админцентр", "index.php?p=admin", "Это панель управления для SourceBans, где вы можете настроить новых администраторов, добавить новый сервер и т.д.");
    }
...
function BuildBreadcrumbs()
{
    ...
    if (isset($_GET['c'])) {
        switch ($_GET['c']) {
            case "admins":
                $cat = "Управление Админом";
                break;
            case "servers":
                $cat = "Управление сервером";
                break;
            case "bans":
                $cat = "Управление банами";
                break;
            case "comms":
                $cat = "Управление блокировкой связи";
                break;
            case "groups":
                $cat = "Управление группами";
                break;
            case "settings":
                $cat = "SourceBans настройки";
                break;
            case "mods":
                $cat = "Мод управление";
                break;
    ...
    if (!empty($bread)) {
        $text = "&raquo; <a href='index.php?p=home'>Основная</a> &raquo; " . $bread;
    } else {
        $text = "&raquo; <a href='index.php?p=home'>Основная</a>";
    }
...
function BitToString($mask, $masktype=0, $head=true)
{
    ...
    if (($mask & ADMIN_DELETE_ADMINS) !=0 || ($mask & ADMIN_OWNER) !=0) {
        $string .= "&bull; Удаление Админов<br />";
    }
    ...
    if (($mask & ADMIN_BAN_IMPORT) !=0 || ($mask & ADMIN_OWNER) !=0) {
        $string .= "&bull; Импортирование банов<br />";
    }
    ...
    if (($mask & ADMIN_ADD_MODS) !=0 || ($mask & ADMIN_OWNER) !=0) {
        $string .= "&bull; Добавить моды<br />";
    }
    if (($mask & ADMIN_EDIT_MODS) !=0 || ($mask & ADMIN_OWNER) !=0) {
        $string .= "&bull; Редактирование модов<br />";
    }
...
function SmFlagsToSb($flagstring, $head=true)
{
    $string = "";
    if ($head) {
        $string .= "<span style='font-size:10px;color:#1b75d1;'>Сервер-разрешения</span><br>";
    }
    if (empty($flagstring)) {
        $string .= "<i>None</i>";
        return $string;
    ...
    if ((strstr($flagstring, "b") || strstr($flagstring, "z"))) {
        $string .= "&bull; Общий администратор<br />";
    }
    ...
    if ((strstr($flagstring, "n") || strstr($flagstring, "z"))) {
        $string .="&bull; Включение читов<br />";
    }
    if ((strstr($flagstring, "z"))) {
        $string .="&bull; Полный админ<br />";
    }

    if ((strstr($flagstring, "o") || strstr($flagstring, "z"))) {
        $string .="&bull; Пользовательский флаг 1<br />";
    }
    if ((strstr($flagstring, "p") || strstr($flagstring, "z"))) {
        $string .="&bull; Пользовательский флаг 2<br />";
    }
    if ((strstr($flagstring, "q") || strstr($flagstring, "z"))) {
        $string .="&bull; Пользовательский флаг 3<br />";
    }
    if ((strstr($flagstring, "r") || strstr($flagstring, "z"))) {
        $string .="&bull; Пользовательский флаг 4<br />";
    }
    if ((strstr($flagstring, "s") || strstr($flagstring, "z"))) {
        $string .="&bull; Пользовательский флаг 5<br />";
    }
    if ((strstr($flagstring, "t") || strstr($flagstring, "z"))) {
        $string .="&bull; Пользовательский флаг 6<br />";
    }
...
function CreateQuote()
{
    ...
    $quote = array(
		...
		array("Перевод от", "CrazyGhostRider"),
