...
function DoLogin(redir)
{
	...
	{
		$('loginPassword.msg').setHTML('Вы должны ввести свой пароль!');
...
{
	var noPerm = confirm("Вы действительно хотите удалить группу: '" + name +"'?");
	if(noPerm == false)
	{
		return;
	}
...
{
	if(archiv == '2') {
		var noPerm = confirm("Вы действительно хотите восстановить жалобу на '" + name + "' из архива?");
	}
	else if(archiv == '1') {
		var noPerm = confirm("Вы уверены, что хотите переместить жалобу на '" + name +"' в архив?");
	}
	else {
		var noPerm = confirm("Вы действительно хотите удалить жалобу на '" + name +"'?");
	}
	if(noPerm == false)
		return;
...
{
	if(archiv == '2') {
		var noPerm = confirm("Вы уверены, что хотите восстановить протест на '" + name + "' из архива?");
	}
	else if(archiv == '1') {
		var noPerm = confirm("Вы уверены, что хотите переместить протест на '" + name +"' в архив?");
	}
	else {
		var noPerm = confirm("Вы уверены, что хотите удалить протест на '" + name +"'?");
	}
	if(noPerm == false)
	{
		return;
...
{
	var noPerm = confirm("Вы действительно хотите удалить сервер: '" + name +"'?");
	if(noPerm == false)
	{
		return;
	}
...
{
	if(confirm==0) {
		ShowBox('Удалить бан', 'Вы уверены, что хотите удалить бан'+(bulk=="true"?"s":"")+' для '+(bulk=="true"?"тех игроков":"\'"+ name +"\'")+'?', 'blue', '', true);
...
{
	if(popup==1) {
		ShowBox('Причина разбана', '<b>Пожалуйста, дайте краткий комментарий, почему вы собираетесь разбанить '+(bulk=="true"?"тех игроков":"\'"+ name +"\'")+'!</b><br><textarea rows="3" cols="40" name="ureason" id="ureason" style="overflow:auto;"></textarea><br><div id="ureason.msg" class="badentry"></div>', 'blue', '', true);
...
function update_web()
{
	...
	if(document.getElementById('webg').value == "c" || document.getElementById('webg').value == "n") {
		$('web.msg').setHTML('Пожалуйста, подождите...');
...
function update_server_groups()
{
	...
	{
		$('group.msg').setHTML('Пожалуйста, подождите...');
...
function ProcessEditAdminPermissions()
{
	...
	if($('immunity'))
	{
	 	if(IsNumeric($('immunity').value))
			...
			ShowBox("Ошибка", "Иммунитет должен быть численным значением (0-9)", "red", "", true);
	}else
		xajax_EditAdminPerms(aid, Mask, srvMask);
}
...
function ProcessEditGroup(type, name)
{
	...
	if(name == "")
	{
		ShowBox("Ошибка", "Вы должны ввести имя для группы.", "red", "", true);
		$('groupname.msg').innerHTML = 'Вы должны ввести имя для группы.';
...
function ProcessMod()
{
	...
	if(!$('name').value)
	{
		$('name.msg').setHTML('Вы должны ввести название мода, который вы добавляете.');
...
function CheckEmail(type, id)
{
	var err = 0;
	if($('subject').value == "") {
		$('subject.msg').setHTML("Вы должны ввести тему для письма.");
	...
	if($('message').value == "") {
		$('message.msg').setHTML("Вы должны ввести сообщение для электронной почты.");
...
function ClearLogs()
{
	var noPerm = confirm("Вы действительно хотите удалить все записи журнала?");
	if(noPerm == false)
	{
		return;
	}
...
{
	var noPerm = confirm("Вы действительно хотите удалить '" + name +"'?");
	if(noPerm == false)
		return;
	xajax_RemoveMod(id);
}
...
function ShowRehashBox(servers, title, msg, color, redir)
{
	...
	msg = msg + '<br /><hr /><i>Переименование данных администратора и группы на всех связанных серверах...</i><div id="rehashDiv" name="rehashDiv" width="100%"></div>';
	ShowBox(title, msg, color, redir, true);
...
function ProcessComment()
{
	...
	{
		$('commenttext.msg').setHTML('Вы должны ввести свой комментарий');
...
{
	var checkUp = confirm("Вы действительно хотите удалить комментарий?");
	if(checkUp == false)
		return;
...
function TickSelectAll()
{
	...
	if($('tickswitch').value==0) {
		$('tickswitch').value=1;
		$('tickswitch').setProperty('title','Убрать выделение');
		$('tickswitchlink').setProperty('title','Убрать выделение');
		$('tickswitchlink').innerHTML = 'Убрать выделение';
	} else {
		$('tickswitch').value=0;
...
function BulkEdit(action, bankey)
{
	...
	{
		case "U":
			UnbanBan(ids, bankey, "", "Массовый разбан", "1", "true");
		break;
		case "D":
			RemoveBan(ids, bankey, "", "Массовое удаление", "0", "true");
		break;
	}
}
...
{
	var checkUp = confirm("Вы уверены, что хотите забанить всех друзей '"+name+"'?");
	if(checkUp == false)
		return;
	ShowBox("Бан друзей "+name, "Бан всех друзей '"+name+"'.<br />Пожалуйста, подождите...<br />Это может длиться очень долго, в зависимости от количества друзей.", 'blue', '', true);
	$('dialog-control').setStyle('display', 'none');
...
{
	if(popup==1) {
		ShowBox('Отправить сообщение', '<b>Введите сообщение, которое вы хотите отправить, в поле <br>\''+name+'\'.</b><br>Вам нужно включить basechat.smx, поскольку мы используем это<br><i>&lt;sm_psay&gt;</i>.<br><textarea rows="3" cols="40" name="ingamemsg" id="ingamemsg" style="overflow:auto;"></textarea><br><div id="ingamemsg.msg" class="badentry"></div>', 'blue', '', true);
		$('dialog-control').setHTML('<input type="button" name="ingmsg" class="btn ok" onmouseover="ButtonOver(\'ingmsg\')" onmouseout="ButtonOver(\'ingmsg\')" id="ingmsg" value="Отправить сообщение" />&nbsp;<input type="button" onclick="closeMsg(\'\');" name="astop" class="btn cancel" onmouseover="ButtonOver(\'astop\')" onmouseout="ButtonOver(\'astop\')" id="astop" value="Отмена" />');
		$('ingmsg').addEvent('click', function(){OpenMessageBox(sid, name, 0);});
	} else if(popup==0) {
		message = $('ingamemsg').value;
		if(message == "") {
			$('ingamemsg.msg').setHTML("Введите ваше сообщение.");
...
{
	if(conf==0)	{
		ShowBox('Кик игрока', '<b>Вы действительно хотите кикнуть игрока  <br>\''+name+'\'?</b>', 'blue', '', true);
...
{
	$('mapimg.msg').setHTML("Загружено: <b>" + filename + "</b>");
}
...
{
    ShowBox('Просмотр профиля сообщества', 'Создание профиля сообщества для "'+name+'", пожалуйста, подождите...', 'blue', '', true);
    $('dialog-control').setStyle('display', 'none');
...
{
	if(confirm==0) {
		ShowBox('Удалить мут', 'Вы действительно хотите удалить мут для '+ name + '?', 'blue', '', true);
		$('dialog-control').setHTML('<input type="button" onclick="RemoveBlock(\''+id+'\', \''+key+'\', \''+page+'\', \''+addslashes(name.replace(/\'/g,'\\\''))+'\', \'1\''+');" name="rban" class="btn ok" onmouseover="ButtonOver(\'rban\')" onmouseout="ButtonOver(\'rban\')" id="rban" value="Удалить мут" />&nbsp;<input type="button" onclick="closeMsg(\'\');$(\'bulk_action\').options[0].selected=true;" name="astop" class="btn cancel" onmouseover="ButtonOver(\'astop\')" onmouseout="ButtonOver(\'astop\')" id="astop" value="Отмена" />');
	} else if(confirm==1) {
		if(page != "")
			var pagelink = page;
		else
...
{
	if(popup==1) {
		ShowBox('Причина размута(чат)', '<b>Пожалуйста, дайте короткий комментарий, почему вы собираетесь размутить '+"\'"+ name +"\'"+'!</b><br><textarea rows="3" cols="40" name="ureason" id="ureason" style="overflow:auto;"></textarea><br><div id="ureason.msg" class="badentry"></div>', 'blue', '', true);
		$('dialog-control').setHTML('<input type="button" onclick="UnGag(\''+id+'\', \''+key+'\', \''+page+'\', \''+addslashes(name.replace(/\'/g,'\\\''))+'\', \'0\''+');" name="uban" class="btn ok" onmouseover="ButtonOver(\'uban\')" onmouseout="ButtonOver(\'uban\')" id="uban" value="Размут(чат) Игрока" />&nbsp;<input type="button" onclick="closeMsg(\'\');" name="astop" class="btn cancel" onmouseover="ButtonOver(\'astop\')" onmouseout="ButtonOver(\'astop\')" id="astop" value="Отмена" />');
	} else if(popup==0) {
		if(page != "")
			var pagelink = page;
		else
...
{
	if(popup==1) {
		ShowBox('Причина размута(микр)', '<b>Пожалуйста, дайте короткий комментарий, почему вы собираетесь размутить '+"\'"+ name +"\'"+'!</b><br><textarea rows="3" cols="40" name="ureason" id="ureason" style="overflow:auto;"></textarea><br><div id="ureason.msg" class="badentry"></div>', 'blue', '', true);
		$('dialog-control').setHTML('<input type="button" onclick="UnMute(\''+id+'\', \''+key+'\', \''+page+'\', \''+addslashes(name.replace(/\'/g,'\\\''))+'\', \'0\''+');" name="uban" class="btn ok" onmouseover="ButtonOver(\'uban\')" onmouseout="ButtonOver(\'uban\')" id="uban" value="Размут(микр) Игрока" />&nbsp;<input type="button" onclick="closeMsg(\'\');" name="astop" class="btn cancel" onmouseover="ButtonOver(\'astop\')" onmouseout="ButtonOver(\'astop\')" id="astop" value="Отмена" />');
	} else if(popup==0) {
		if(page != "")
			var pagelink = page;
		else
...
{
	ShowBox('Мут добавлен', 'Мут успешно добавлен<br><iframe id="srvkicker" frameborder="0" width="100%" src="pages/admin.blockit.php?check='+check+'&type='+type+'&length='+length+'"></iframe>', 'green', 'index.php?p=admin&c=comms', true);
}
