...
if (isset($_POST['postd']) && $_POST['postd']) {
    if (empty($_POST['uname']) ||empty($_POST['pass1']) ||empty($_POST['pass2'])||empty($_POST['steam'])||empty($_POST['email'])) {
        ...
        if (!$db) {
            echo "<script>ShowBox('Ошибка', 'При подключении к базе данных произошла ошибка. <br />Повторно проверьте данные, чтобы убедиться, что они верны', 'red', '', true);</script>";
        } else {
            // Setup Admin
...
?>
    ...
    <div id="submit-main">
        <div align="center">
        Последний шаг - добавить это в свои databases.cfg на своем игровом сервере (/[MOD]/addons/sourcemod/configs/databases.cfg)<br />
        Этот код должен быть добавлен <b>ВНУТРИ</b>  `"Databases" { [вставить здесь] }` части файла.<br />
        <textarea cols="105" rows="14" readonly><?php echo $srv_cfg;?></textarea>

        <?php
        if (strtolower($_POST['server']) == "localhost") {
            echo '<script>ShowBox("Предупреждение локального сервера", "Вы сказали, что ваш сервер MySQL работает в том же поле, что и веб-сервер, это нормально, но вам может потребоваться изменить следующую конфигурацию, чтобы установить удаленный домен / ip вашего сервера MySQL. Если ваш игровой сервер находится в том же поле, что и ваш веб-сервер." , "blue", "", true);</script>';
        }
        if (!is_writable("../config.php")) {
        ?>
            <br /><br />
            Поскольку ваш config.php недоступен для записи сервером, вам необходимо добавить следующее в файл (./config.php).
    ...
    <table style="width: 101%; margin: 0 0 -2px -2px;">
        <tr>
            <td colspan="3" class="listtable_top"><b>Завершено</b></td>
        </tr>
    </table>
    <div id="submit-main">
        <div align="center">
        Настройка SourceBans завершена. Удалите папку "install", чтобы завершить установку. <br />
        <i>Если вам нужно импортировать запреты из AMXBans, нажмите кнопку импорта ниже</i><br/><br/>
...
<table style="width: 101%; margin: 0 0 -2px -2px;">
    <tr>
        <td colspan="3" class="listtable_top"><b>Настройки</b></td>
    </tr>
</table>
...
<table width="60%" style="border-collapse:collapse;" id="group.details" cellpadding="3">
  <tr>
    <td valign="top" width="35%"><div class="rowdesc"><?php echo HelpIcon("Основной администратор", "Введите имя пользователя для основного администратора SourceBans");?>Имя администратора</div></td>
    <td><div align="center">
  	 <input type="text" TABINDEX=1 class="textbox" id="uname" name="uname" value="" />
    </div><div id="server.msg" style="color:#CC0000;"></div></td>
  </tr>
  <tr>
    <td valign="top" width="35%"><div class="rowdesc"><?php echo HelpIcon("Пароль", "Введите пароль для главного администратора");?>Пароль администратора</div></td>
    <td><div align="center">
  	 <input type="password" TABINDEX=1 class="textbox" id="pass1" name="pass1" value="" />
    </div><div id="port.msg" style="color:#CC0000;"></div></td>
  </tr>
  <tr>
    <td valign="top" width="35%"><div class="rowdesc"><?php echo HelpIcon("Подтвердить", "Введите пароль еще раз");?>Подтвердите Пароль</div></td>
    <td><div align="center">
  	 <input type="password" TABINDEX=1 class="textbox" id="pass2" name="pass2" value="" />
    </div><div id="user.msg" style="color:#CC0000;"></div></td>
  </tr>
  <tr>
    <td valign="top" width="35%"><div class="rowdesc"><?php echo HelpIcon("STEAM", "Введите свой STEAM id");?>Steam ID</div></td>
    <td><div align="center">
  	 <input type="text" TABINDEX=1 class="textbox" id="steam" name="steam" value="" />
    </div><div id="user.msg" style="color:#CC0000;"></div></td>
  </tr>
  <tr>
    <td valign="top" width="35%"><div class="rowdesc"><?php echo HelpIcon("Email", "Введите адрес электронной почты");?>Email</div></td>
    <td><div align="center">
...
function CheckInput()
{
	...
	if(error > 0)
		ShowBox('Ошибка', 'Вы должны заполнить все поля на этой странице.', 'red', '', true);
	else
		$('mfrm').submit();
}
</script>
