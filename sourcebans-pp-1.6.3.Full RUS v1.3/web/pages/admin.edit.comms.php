...
if ($_GET['key'] != $_SESSION['banlist_postkey']) {
    echo '<script>ShowBox("Ошибка", "Возможная попытка взлома (несоответствие ключа URL)!", "red", "index.php?p=admin&c=comms");</script>';
    PageDie();
}
if (!isset($_GET['id']) || !is_numeric($_GET['id'])) {
    echo '<script>ShowBox("Ошибка", "Идентификатор блока не указан. Пожалуйста, следуйте ссылкам!", "red", "index.php?p=admin&c=comms");</script>';
    PageDie();
}
...
if (!$userbank->HasAccess(ADMIN_OWNER | ADMIN_EDIT_ALL_BANS) && (!$userbank->HasAccess(ADMIN_EDIT_OWN_BANS) && $res[8] != $userbank->GetAid()) && (!$userbank->HasAccess(ADMIN_EDIT_GROUP_BANS) && $res->fields['gid'] != $userbank->GetProperty('gid'))) {
    echo '<script>ShowBox("Ошибка", "У тебя нет доступа к этому!", "red", "index.php?p=admin&c=comms");</script>';
    PageDie();
}
...
if (isset($_POST['name'])) {
    ...
    if ($error == 0) {
        ...
        if ($_POST['banlength'] != $lengthrev->fields['length']) {
            $log = new CSystemLog("m", "Мут редактирован", "Мут для (" . $lengthrev->fields['authid'] . ") обновлен, раньше: срок " . $lengthrev->fields['length'] . ", тип " . $lengthrev->fields['type'] . "; сейчас: срок " . $_POST['banlength'] . " тип " . $_POST->fields['type']);
        }
        echo '<script>ShowBox("Мут обновлен", "Мут успешно обновлен", "green", "index.php?p=commslist' . $pagelink . '");</script>';
    }
}

if (!$res) {
    echo '<script>ShowBox("Ошибка", "Произошла ошибка получения информации. Может быть, мут удален?", "red", "index.php?p=commslist' . $pagelink . '");</script>';
}
