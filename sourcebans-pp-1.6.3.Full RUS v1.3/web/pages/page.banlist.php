...
if (isset($_GET['a']) && $_GET['a'] == "unban" && isset($_GET['id'])) {
    ...
    foreach ($bids as $bid) {
        ...
        if (empty($row) || !$row) {
            $fail++;
            if (!isset($_GET['bulk'])) {
                echo "<script>ShowBox('Игрок не разбанен', 'Игрок не был разбанен, либо уже разбанен, либо с не действительным баном.', 'red', 'index.php?p=banlist$pagelink');</script>";
                PageDie();
            }
            continue;
        }
        ...
        if ($res) {
            if (!isset($_GET['bulk'])) {
                echo "<script>ShowBox('Игрок разбанен', '" . StripQuotes($row['name']) . " (" . ($row['type'] == 0 ? $row['authid'] : $row['ip']) . ") был разбанен из SourceBans.', 'green', 'index.php?p=banlist$pagelink');</script>";
            }
            $log = new CSystemLog("m", "Игрок разбанен", "'" . StripQuotes($row['name']) . "' (" . ($row['type'] == 0 ? $row['authid'] : $row['ip']) . ") был разбанен");
            $ucount++;
        } else {
            if (!isset($_GET['bulk'])) {
                echo "<script>ShowBox('Игрок НЕ разбанен', 'Была ошибка разбанивания " . StripQuotes($row['name']) . "', 'red', 'index.php?p=banlist$pagelink', true);</script>";
            }
            $fail++;
        }
    }
    if (isset($_GET['bulk'])) {
        echo "<script>ShowBox('Игроки разбанены', '$ucount игроки были разбанены из SourceBans.<br>$fail failed.', 'green', 'index.php?p=banlist$pagelink');</script>";
    }
    ...
    if (!$userbank->HasAccess(ADMIN_OWNER | ADMIN_DELETE_BAN)) {
        echo "<script>ShowBox('Ошибка', 'У вас нет доступа к этому.', 'red', 'index.php?p=banlist$pagelink');</script>";
        PageDie();
    }
    ...
    foreach ($bids as $bid) {
        ...
        if ($res) {
            if (!isset($_GET['bulk'])) {
                echo "<script>ShowBox('Бан удален', 'Бан \'" . StripQuotes($steam['name']) . "\' (" . ($steam['type'] == 0 ? $steam['authid'] : $steam['ip']) . ") был удален из SourceBans', 'green', 'index.php?p=banlist$pagelink');</script>";
            }
            $log = new CSystemLog("m", "Бан удален", "Бан '" . StripQuotes($steam['name']) . "' (" . ($steam['type'] == 0 ? $steam['authid'] : $steam['ip']) . ") был удален.");
            $dcount++;
        } else {
            if (!isset($_GET['bulk'])) {
                echo "<script>ShowBox('Бан НЕ удален', 'Бан \'" . StripQuotes($steam['name']) . "\' произошла ошибка во время удаления', 'red', 'index.php?p=banlist$pagelink', true);</script>";
            }
            $fail++;
        }
    }
    if (isset($_GET['bulk'])) {
        echo "<script>ShowBox('Игроки удалены', '$dcount игроки были удалены из SourceBans.<br>$fail failed.', 'green', 'index.php?p=banlist$pagelink');</script>";
    }
}
...
while (!$res->EOF) {
    ...
    if (!empty($res->fields['ban_ip'])) {
        if (!empty($res->fields['ban_country']) && $res->fields['ban_country'] != ' ') {
            ...
            $data['country'] = '<img src="images/country/zz.jpg" alt="Неизвестная страна" border="0" align="absmiddle" />';
        }
    } else {
        $data['country'] = '<img src="images/country/zz.jpg" alt="Неизвестная страна" border="0" align="absmiddle" />';
    }
    ...
    $data['blockcomm_link']  = CreateLinkR('<img src="images/forbidden.png" border="0" alt="" style="vertical-align:middle" /> Мутнуть', "index.php?p=admin&c=comms" . $pagelink . "&blockfromban=" . $res->fields['ban_id'] . "&key=" . $_SESSION['banlist_postkey'] . "#^0");
    $data['details_link']    = CreateLinkR('click', 'getdemo.php?type=B&id=' . $res->fields['ban_id']);
    $data['groups_link']     = CreateLinkR('<img src="images/groups.png" border="0" alt="" style="vertical-align:middle" /> Посмотреть группы', "index.php?p=admin&c=bans&fid=" . $data['communityid'] . "#^4");
    ...
    $data['edit_link']       = CreateLinkR('<img src="images/edit.png" border="0" alt="" style="vertical-align:middle" /> Редактировать', "index.php?p=admin&c=bans&o=edit" . $pagelink . "&id=" . $res->fields['ban_id'] . "&key=" . $_SESSION['banlist_postkey']);

    $data['unban_link']  = CreateLinkR('<img src="images/locked.png" border="0" alt="" style="vertical-align:middle" /> Разбанить', "#", "", "_self", false, "UnbanBan('" . $res->fields['ban_id'] . "', '" . $_SESSION['banlist_postkey'] . "', '" . $pagelink . "', '" . StripQuotes($data['player']) . "', 1, false);return false;");
    $data['delete_link'] = CreateLinkR('<img src="images/delete.png" border="0" alt="" style="vertical-align:middle" /> Удалить бан', "#", "", "_self", false, "RemoveBan('" . $res->fields['ban_id'] . "', '" . $_SESSION['banlist_postkey'] . "', '" . $pagelink . "', '" . StripQuotes($data['player']) . "', 0, false);return false;");
    ...
    if ($res->fields['demo_count'] == 0) {
        ...
        $data['demo_quick']     = 'N/A';
        $data['demo_link']      = CreateLinkR('<img src="images/demo.png" border="0" alt="" style="vertical-align:middle" /> Нет демо', "#");
    } else {
        $data['demo_available'] = true;
        $data['demo_quick']     = CreateLinkR('Demo', "getdemo.php?type=B&id=" . $data['ban_id']);
        $data['demo_link']      = CreateLinkR('<img src="images/demo.png" border="0" alt="" style="vertical-align:middle" /> Обзор демо', "getdemo.php?type=B&id=" . $data['ban_id']);
    }
    ...
    foreach ($banlog as $logged) {
        ...
        $logstring .= '<span title="Сервер: ' . $logged["ip"] . ':' . $logged["port"] . ', Дата: ' . date($dateformat, $logged["time"]) . '">' . ($logged["name"] != "" ? htmlspecialchars($logged["name"]) : "<i>нет имени</i>") . '</span>';
    }
        ...
        if ($commentres->RecordCount() > 0) {
            ...
            while (!$commentres->EOF) {
                ...
                if ($commentres->fields['aid'] == $userbank->GetAid() || $userbank->HasAccess(ADMIN_OWNER)) {
                    $cdata['editcomlink'] = CreateLinkR('<img src=\'images/edit.png\' border=\'0\' alt=\'\' style=\'vertical-align:middle\' />', 'index.php?p=banlist&comment=' . $data['ban_id'] . '&ctype=B&cid=' . $commentres->fields['cid'] . $pagelink, 'Изменить комментарий');
                    if ($userbank->HasAccess(ADMIN_OWNER)) {
                        $cdata['delcomlink'] = "<a href=\"#\" class=\"tip\" title=\"<img src='images/delete.png' border='0' alt='' style='vertical-align:middle' /> :: Удалить комментарий\" target=\"_self\" onclick=\"RemoveComment(" . $commentres->fields['cid'] . ",'B'," . (isset($_GET["page"]) ? $page : -1) . ");\"><img src='images/delete.png' border='0' alt='' style='vertical-align:middle' /></a>";
                    }
                } else {
                    $cdata['editcomlink'] = "";
                    $cdata['delcomlink']  = "";
    ...
    $data['addcomment'] = CreateLinkR('<img src="images/details.png" border="0" alt="" style="vertical-align:middle" /> Добавить комментарий', 'index.php?p=banlist&comment=' . $data['ban_id'] . '&ctype=B' . $pagelink);
    //-----------------------------------
