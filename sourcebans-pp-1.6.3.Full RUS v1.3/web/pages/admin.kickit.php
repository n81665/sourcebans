...
function LoadServers($check, $type)
{
    ...
    if (!$userbank->HasAccess(ADMIN_OWNER | ADMIN_ADD_BAN)) {
        ...
        $log = new CSystemLog("w", "Попытка взлома", $username . " tried to use kickit, but doesnt have access.");
        return $objResponse;
    }
    $id      = 0;
    ...
    while (!$servers->EOF) {
        ...
        if (!empty($servers->fields["rcon"])) {
            ...
            $text = '<font size="1">Нет пароля rcon.</font>';
...
function KickPlayer($check, $sid, $num, $type)
{
    ...
    if (!$userbank->HasAccess(ADMIN_OWNER | ADMIN_ADD_BAN)) {
        $objResponse->redirect("index.php?p=login&m=no_access", 0);
        $log = new CSystemLog("w", "Попытка взлома", $username . " tried to process a playerkick, but doesnt have access.");
        return $objResponse;
    }
