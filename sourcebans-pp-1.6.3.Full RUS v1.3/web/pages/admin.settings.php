...
if (isset($_GET['log_clear']) && $_GET['log_clear'] == "true") {
    if ($userbank->HasAccess(ADMIN_OWNER)) {
        ...
        $log = new CSystemLog("w", "Попытка взлома", $userbank->GetProperty('user') . " попытался очистить журналы, но не имеет доступа.");
    }
}
...
if (!$userbank->HasAccess(ADMIN_OWNER | ADMIN_WEB_SETTINGS)) {
    echo '<div id="0" style="display:none;">Доступ закрыт!</div>';
} else {
    if (isset($_POST['settingsGroup'])) {
        ...
        if ($_POST['settingsGroup'] == "mainsettings") {
            if (!is_numeric($_POST['config_password_minlength'])) {
                $errors .= "Минимальная длина пароля должна быть числом<br />";
            }
            if (!is_numeric($_POST['banlist_bansperpage'])) {
                $errors .= "Запреты на страницу должны быть числом";
            }
            if (empty($errors)) {
...
?>
<script>ShowBox('Настройки обновлены', 'Изменения были успешно обновлены', 'green', 'index.php?p=admin&c=settings');</script>
<?php
            } else {
...
<?php
    ...
    if ($userbank->HasAccess(ADMIN_OWNER)) {
        $theme->assign('clear_logs', "( <a href='javascript:ClearLogs();'>Очистить журнал</a> )");
    }
...
if (ini_get('safe_mode') == 1) {
    ...
    print "$('enable_friendsbanning.msg').setHTML('Вы не можете использовать эти функции. Вам нужно установить выключить безопасный режим PHP.');\n";
