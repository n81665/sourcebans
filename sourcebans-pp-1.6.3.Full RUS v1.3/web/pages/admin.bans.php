...
if (isset($_POST['action']) && $_POST['action'] == "importBans") {
    ...
    echo "<script>ShowBox('Импорт банов', '$bancnt ban" . ($bancnt != 1 ? "s have" : " has") . " были импортированы и отправлены.', 'green', '');</script>";
}
...
if ((isset($_GET['action']) && $_GET['action'] == "pasteBan") && isset($_GET['pName']) && isset($_GET['sid'])) {
    echo "<script type=\"text/javascript\">ShowBox('Loading..','<b>Loading...</b><br><i>Пожалуйста, подождите!</i>', 'blue', '', true);document.getElementById('dialog-control').setStyle('display', 'none');xajax_PasteBan('" . (int) $_GET['sid'] . "', '" . addslashes($_GET['pName']) . "');</script>";
}
...
echo '<div id="tabsWrapper" style="margin:0px;">
	...
	<ul>
		<li id="utab-p0" class="active">
			<a href="index.php?p=admin&c=bans#^1~p0" id="admin_utab_p0" onclick="Swap2ndPane(0,\'p\');" class="tip" title="Показать протесты :: Показать текущие протесты." target="_self">Текущие</a>
		</li>
...
$page_nav = 'отображено&nbsp;' . $PageStart . '&nbsp;-&nbsp;' . $PageEnd . '&nbsp;из&nbsp;' . $page_count . '&nbsp;результатов';
...
foreach ($protestsarchiv as $prot) {
    ...
    if ($prot['archiv'] != "2") {
        ...
        if (!$protestb) {
            ...
            if ($prot['archiv'] == "1") {
                $prot['archive'] = "протест был заархивирован.";
            } elseif ($prot['archiv'] == "3") {
                $prot['archive'] = "срок действия бан истек.";
            } elseif ($prot['archiv'] == "4") {
                $prot['archive'] = "Бан был разбанен.";
            }
        }
    } else {
        $prot['archive'] = "бан был удален.";
    ...
    if ($commentres->RecordCount() > 0) {
        ...
        while (!$commentres->EOF) {
            ...
            $cdata['morecom'] = ($morecom == 1 ? true : false);
            if ($commentres->fields['aid'] == $userbank->GetAid() || $userbank->HasAccess(ADMIN_OWNER)) {
                $cdata['editcomlink'] = CreateLinkR('<img src=\'images/edit.png\' border=\'0\' alt=\'\' style=\'vertical-align:middle\' />', 'index.php?p=banlist&comment=' . (int) $prot['pid'] . '&ctype=P&cid=' . $commentres->fields['cid'], 'Изменить комментарий');
                if ($userbank->HasAccess(ADMIN_OWNER)) {
                    $cdata['delcomlink'] = "<a href=\"#\" class=\"tip\" title=\"<img src='images/delete.png' border='0' alt='' style='vertical-align:middle' /> :: Удалить комментарий\" target=\"_self\" onclick=\"RemoveComment(" . $commentres->fields['cid'] . ",'P',-1);\"><img src='images/delete.png' border='0' alt='' style='vertical-align:middle' /></a>";
                }
            } else {
                $cdata['editcomlink'] = "";
                $cdata['delcomlink']  = "";
...
echo '<div id="tabsWrapper" style="margin:0px;">
	...
	<ul>
		<li id="utab-s0" class="active">
			<a href="index.php?p=admin&c=bans#^2~s0" id="admin_utab_s0" onclick="Swap2ndPane(0,\'s\');" class="tip" title="Показать жалобы :: Показать текущие жалобы." target="_self">Текущие</a>
		</li>
		<li id="utab-s1" class="nonactive">
			<a href="index.php?p=admin&c=bans#^2~s1" id="admin_utab_s1" onclick="Swap2ndPane(1,\'s\');" class="tip" title="Показать архив :: Показать архив жалоб." target="_self">Архив</a>
		</li>
	</ul>
	</div>
	</div>';
...
foreach ($submissions as $sub) {
    ...
    if ($commentres->RecordCount() > 0) {
        ...
        while (!$commentres->EOF) {
            ...
            $cdata['morecom'] = ($morecom == 1 ? true : false);
            if ($commentres->fields['aid'] == $userbank->GetAid() || $userbank->HasAccess(ADMIN_OWNER)) {
                $cdata['editcomlink'] = CreateLinkR('<img src=\'images/edit.png\' border=\'0\' alt=\'\' style=\'vertical-align:middle\' />', 'index.php?p=banlist&comment=' . (int) $sub['subid'] . '&ctype=S&cid=' . $commentres->fields['cid'], 'Изменить комментарий');
                if ($userbank->HasAccess(ADMIN_OWNER)) {
                    $cdata['delcomlink'] = "<a href=\"#\" class=\"tip\" title=\"<img src='images/delete.png' border='0' alt='' style='vertical-align:middle' /> :: Удалить комментарий\" target=\"_self\" onclick=\"RemoveComment(" . $commentres->fields['cid'] . ",'S',-1);\"><img src='images/delete.png' border='0' alt='' style='vertical-align:middle' /></a>";
                }
            } else {
                $cdata['editcomlink'] = "";
                $cdata['delcomlink']  = "";
...
foreach ($submissionsarchiv as $sub) {
    ...
    if ($sub['archiv'] == "3") {
        ...
        $sub['archive'] = "Жалоба принята.";
    } elseif ($sub['archiv'] == "1") {
        $sub['archive'] = "Жалоба архивирована.";
    }
    //COMMENT STUFF
    //-----------------------------------
    $view_comments = true;
...
function ProcessBan()
{
    ...
    if (!$('nickname').value) {
        $('nick.msg').setHTML('Вы должны ввести Ник игрока, которого вы баните');
    ...
    if (!reason) {
        $('reason.msg').setHTML('Вы должны выбрать или ввести причину этого бана.');
...
{
    if (!$('groupurl').value) {
        $('groupurl.msg').setHTML('Вы должны ввести ссылку группы, которую вы баните');
