...
if (isset($_GET["rebanid"])) {
    ...
    echo "<script type=\"text/javascript\">ShowBox('Загрузка..','<b>Загрузка...</b><br><i>Пожалуйста, подождите!</i>', 'blue', '', true);document.getElementById('dialog-control').setStyle('display', 'none');xajax_PasteBlock('" . (int) $_GET['sid'] . "', '" . addslashes($_GET['pName']) . "');</script>";
}
...
function ProcessBan()
{
    ...
    if(!reason)
    {
        $('reason.msg').setHTML('Вы должны выбрать или ввести причину для этого мута.');
