...
if (!isset($_GET['id'])) {
	...
	<b>Ошибка</b>
	<br />
	Идентификатор сервера не указан. Пожалуйста, следуйте ссылкам
</div>';
    die();
}
...
if (!isset($_GET['type']) || ($_GET['type'] != 'web' && $_GET['type'] != 'srv' && $_GET['type'] != 'server')) {
	...
	<b>Ошибка</b>
	<br />
	Не указан допустимый тип группы. Пожалуйста, следуйте ссылкам
</div>';
    die();
}
...
<table width="90%" style="border-collapse:collapse;" id="group.details" cellpadding="3">
  <tr>
    <td valign="top" width="35%"><div class="rowdesc"><?=HelpIcon("Название группы", "Введите имя новой группы, которую вы хотите создать.")?>Название группы </div></td>
    <td><div align="left">
...
Групповые переопределения позволяют разрешать или отклонять определенные команды или группы команд членам группы.<br />
<i>Прочитать о <b><a href="http://wiki.alliedmods.net/Adding_Groups_%28SourceMod%29" title="Добавление групповых (SourceMod)" target="_blank">переопределений групп</a></b> в AlliedModders Wiki!</i><br /><br />
    ...
    <tr>
        <td class="tablerow1">
            <select name="override_type[]">
                <option value="command" <?=$override['type'] == "command" ? "selected=\"selected\"" : ""?>>Команда</option>
                <option value="group"<?=$override['type'] == "group" ? "selected=\"selected\"" : ""?>>Группа</option>
            </select>
            ...
            <td class="tablerow1">
                <select name="override_access[]">
                    <option value="allow"<?=$override['access'] == "allow" ? "selected=\"selected\"" : ""?>>Позволять</option>
                    <option value="deny"<?=$override['access'] == "deny" ? "selected=\"selected\"" : ""?>>Отрицать</option>
                </select>
            </td>
    </tr>
<?php
    ...
    <tr>
        ...
        <td class="tablerow1">
            <select id="new_override_access">
                <option value="allow">Позволять</option>
                <option value="deny">Отрицать</option>
            </select>
        </td>
    </tr>
</table>
...
<table width="100%">
    ...
    <tr align="center">
        ...
        <td>
            <div align="center">
        <?=$ui->drawButton("Сохранить изменения", "ProcessEditGroup('" . $_GET['type'] . "', $('groupname').value);", "ok", "editgroup", true)?>
