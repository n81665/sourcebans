...
if (!$userbank->HasAccess(ADMIN_OWNER | ADMIN_ADD_SERVER)) {
    $log = new CSystemLog("w", "Попытка взлома", $userbank->GetProperty('user') . " пытался загрузить карту, но не имеет доступа.");
...
if (isset($_POST['upload'])) {
    if (CheckExt($_FILES['mapimg_file']['name'], "jpg")) {
        ...
        $log     = new CSystemLog("m", "Загрузка изображения карты", "Загружено новое изображение карты: " . htmlspecialchars($_FILES['mapimg_file']['name']));
    } else {
        $message = "<b> Файл должен быть jpg.</b><br><br>";
    }
}

$theme->assign("title", "Загрузить изображение");
