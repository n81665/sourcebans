...
Licensed under GNU GPL version 3, or later. CGR
...
if ($GLOBALS['config']['config.enablecomms'] != "1") {
    CreateRedBox("Ошибка", "Эта страница отключена. Вас не должны быть здесь.");
    PageDie();
}
...
if (isset($_GET['a']) && $_GET['a'] == "ungag" && isset($_GET['id'])) {
    if ($_GET['key'] != $_SESSION['banlist_postkey']) {
        die("Возможная попытка взлома (несоответствие ключа URL)");
    }
    ...
    if (empty($row) || !$row) {
        echo "<script>ShowBox('Игрок НЕ размучен(Чат)', 'Игрок не был размучен(Чат), либо был размучен(Чат) или не имел мута.', 'red', 'index.php?p=commslist$pagelink');</script>";
        PageDie();
    }
    ...
    if ($res) {
        echo "<script>ShowBox('Игрок размучен(Чат)', '" . StripQuotes($row['name']) . " (" . $row['authid'] . ") был размучен(Чат) из SourceBans.', 'green', 'index.php?p=commslist$pagelink');</script>";
        $log = new CSystemLog("m", "Игрок размучен(Чат)", "'" . StripQuotes($row['name']) . "' (" . $row['authid'] . ") был размучен(Чат)");
    } else {
        echo "<script>ShowBox('Игрок НЕ размучен(Чат)', 'Произошла ошибка при размучивании(Чат) " . StripQuotes($row['name']) . "', 'red', 'index.php?p=commsist$pagelink', true);</script>";
    }
    ...
    if (empty($row) || !$row) {
        echo "<script>ShowBox('Игрок НЕ размучен(Микрофон)', 'Игрок не был размучен(Микрофон), либо был размучен(Микрофон) или не имел мута.', 'red', 'index.php?p=commslist$pagelink');</script>";
        PageDie();
    }
    ...
    if ($res) {
        echo "<script>ShowBox('Игрок размучен(Микрофон)', '" . StripQuotes($row['name']) . " (" . $row['authid'] . ") был размучен(Микрофон) из SourceBans.', 'green', 'index.php?p=commslist$pagelink');</script>";
        $log = new CSystemLog("m", "Игрок размучен(Микрофон)", "'" . StripQuotes($row['name']) . "' (" . $row['authid'] . ") был размучен(Микрофон)");
    } else {
        echo "<script>ShowBox('Игрок НЕ размучен(Микрофон)', 'Произошла ошибка при размучивании(Микрофон) " . StripQuotes($row['name']) . "', 'red', 'index.php?p=commsist$pagelink', true);</script>";
    }
    ...
    if (!$userbank->HasAccess(ADMIN_OWNER | ADMIN_DELETE_BAN)) {
        echo "<script>ShowBox('Ошибка', 'У вас нет доступа к этому.', 'red', 'index.php?p=commslist$pagelink');</script>";
        PageDie();
    }
    ...
    if ($res) {
        echo "<script>ShowBox('Мут удален', 'Мут \'" . StripQuotes($steam['name']) . "\' (" . $steam['authid'] . ") был удален из SourceBans', 'green', 'index.php?p=commslist$pagelink');</script>";
        $log = new CSystemLog("m", "Мут удален", "Мут '" . StripQuotes($steam['name']) . "' (" . $steam['authid'] . ") был удален.");
    } else {
        echo "<script>ShowBox('Мут НЕ удален', 'Мут \'" . StripQuotes($steam['name']) . "\' имел ошибку при удалении.', 'red', 'index.php?p=commslist$pagelink', true);</script>";
    }
}
...
while (!$res->EOF) {
    ...
    if ($alrdybnd->fields['count'] == 0) {
        switch ($data['type']) {
            case 1:
                $data['reban_link'] = CreateLinkR('<img src="images/forbidden.png" border="0" alt="" style="vertical-align:middle" /> Перезамутить(Микро)', "index.php?p=admin&c=comms" . $pagelink . "&rebanid=" . $res->fields['ban_id'] . "&key=" . $_SESSION['banlist_postkey'] . "#^0");
                break;
            case 2:
                $data['reban_link'] = CreateLinkR('<img src="images/forbidden.png" border="0" alt="" style="vertical-align:middle" /> Перезамутить(Чат)', "index.php?p=admin&c=comms" . $pagelink . "&rebanid=" . $res->fields['ban_id'] . "&key=" . $_SESSION['banlist_postkey'] . "#^0");
                break;
            default:
                break;
        }
    ...
    $data['edit_link'] = CreateLinkR('<img src="images/edit.png" border="0" alt="" style="vertical-align:middle" /> Редактировать', "index.php?p=admin&c=comms&o=edit" . $pagelink . "&id=" . $res->fields['ban_id'] . "&key=" . $_SESSION['banlist_postkey']);

    switch ($data['type']) {
        case 2:
            $data['unban_link'] = CreateLinkR('<img src="images/locked.png" border="0" alt="" style="vertical-align:middle" /> Размутить(Чат)', "#", "", "_self", false, "UnGag('" . $res->fields['ban_id'] . "', '" . $_SESSION['banlist_postkey'] . "', '" . $pagelink . "', '" . StripQuotes($data['player']) . "', 1);return false;");
            break;
        case 1:
            $data['unban_link'] = CreateLinkR('<img src="images/locked.png" border="0" alt="" style="vertical-align:middle" /> Размутить(Микро)', "#", "", "_self", false, "UnMute('" . $res->fields['ban_id'] . "', '" . $_SESSION['banlist_postkey'] . "', '" . $pagelink . "', '" . StripQuotes($data['player']) . "', 1);return false;");
            break;
        default:
            break;
    }

    $data['delete_link'] = CreateLinkR('<img src="images/delete.png" border="0" alt="" style="vertical-align:middle" /> Удалить мут', "#", "", "_self", false, "RemoveBlock('" . $res->fields['ban_id'] . "', '" . $_SESSION['banlist_postkey'] . "', '" . $pagelink . "', '" . StripQuotes($data['player']) . "', 0);return false;");
    ...
    if ($history_count > 1) {
        ...
        $data['prevoff_link'] = "Нет предыдущих мутов";
    }
        ...
        if ($commentres->RecordCount() > 0) {
            ...
            while (!$commentres->EOF) {
                ...
                if ($commentres->fields['aid'] == $userbank->GetAid() || $userbank->HasAccess(ADMIN_OWNER)) {
                    $cdata['editcomlink'] = CreateLinkR('<img src=\'images/edit.png\' border=\'0\' alt=\'\' style=\'vertical-align:middle\' />', 'index.php?p=commslist&comment=' . $data['ban_id'] . '&ctype=C&cid=' . $commentres->fields['cid'] . $pagelink, 'Изменить комментарий');
                    if ($userbank->HasAccess(ADMIN_OWNER)) {
                        $cdata['delcomlink'] = "<a href=\"#\" class=\"tip\" title=\"<img src='images/delete.png' border='0' alt='' style='vertical-align:middle' /> :: Удалить комментарий\" target=\"_self\" onclick=\"RemoveComment(" . $commentres->fields['cid'] . ",'C'," . (isset($_GET["page"]) ? $_GET["page"] : -1) . ");\"><img src='images/delete.png' border='0' alt='' style='vertical-align:middle' /></a>";
                    }
                } else {
                    $cdata['editcomlink'] = "";
                    $cdata['delcomlink']  = "";
    ...
    $data['addcomment'] = CreateLinkR('<img src="images/details.png" border="0" alt="" style="vertical-align:middle" /> Добавить комментарий', 'index.php?p=commslist&comment=' . $data['ban_id'] . '&ctype=C' . $pagelink);
    //-----------------------------------
...
//=================[ Start Layout ]==================================
$ban_nav = 'отображено&nbsp;' . $BansStart . '&nbsp;-&nbsp;' . $BansEnd . '&nbsp;из&nbsp;' . $BanCount . '&nbsp;результатов';
...
if (isset($_GET["comment"])) {
    $theme->assign('commenttype', (isset($_GET["cid"]) ? "Изменить" : "Добавить"));
    if (isset($_GET["cid"])) {
