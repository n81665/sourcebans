...
foreach ($admins as $admin) {
    ...
    if (empty($admin['web_group']) || $admin['web_group'] == " ") {
        $admin['web_group'] = "Нет групповых/индивидуальных разрешений";
    }
    if (empty($admin['server_group']) || $admin['server_group'] == " ") {
        $admin['server_group'] = "Нет групповых/индивидуальных разрешений";
    }
...
//=================[ Start Layout ]==================================
$admin_nav = 'отображено&nbsp;' . $AdminsStart . '&nbsp;-&nbsp;' . $AdminsEnd . '&nbsp;из&nbsp;' . $admin_count . '&nbsp;результатов';
    ...
    if (isset($_POST['new_override_name'])) {
        ...
        if (isset($_POST['override_id'])) {
            ...
            foreach ($_POST['override_id'] as $index => $id) {
                ...
                if (!empty($chk)) {
                    $edit_errors .= "&bull; Там уже есть переопределение с именем \\\"" . htmlspecialchars(addslashes($_POST['override_name'][$index])) . "\\\" от выбранного типа.<br />";
                    continue;
                }
            ...
            if (!empty($edit_errors)) {
                throw new Exception("Были ошибки, связанные с вашими изменениями:<br /><br />" . $edit_errors);
            }
        }
        ...
        if (!empty($_POST['new_override_name'])) {
            if ($_POST['new_override_type'] != "command" && $_POST['new_override_type'] != "group") {
                throw new Exception("Недопустимый тип переопределения.");
            }
            ...
            if (!empty($chk)) {
                throw new Exception("Там уже есть переопределение с этим именем из выбранного типа.");
            }
